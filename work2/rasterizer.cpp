...
static bool insideTriangle(int x, int y, const std::array<Eigen::Vector4f, 3>& tri_vertexs)
{   
    // TODO : Implement this function to check if the point (x, y) is inside the triangle represented by _v[0], _v[1], _v[2]
	Eigen::Vector2f v1, v2;
	float pre_z;
	for(int i = 0; i < 3; i++) {
		float x0 = tri_vertexs[i].x(), y0 = tri_vertexs[i].y();
		float x1 = tri_vertexs[(i + 1) % 3].x(), y1 = tri_vertexs[(i + 1) % 3].y();
		v1[0] = x1 - x0, v1[1] = y1 - y0;
		v2[0] = x - x0, v2[1] = y - y0;
		float z = v1[0] * v2[1] - v1[1] * v2[0];
		if(i && z * pre_z < 0) return false;
		pre_z = z;
	}
	return true;
}
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
	...
	Eigen::Vector2i bound_box[2] = {{x_min, y_min},

...
static bool insideTriangle(float x, float y, const std::array<Vector4f, 3>& tri)
{   
    // TODO : Implement this function to check if the point (x, y) is inside the triangle represented by _v[0], _v[1], _v[2]

    Eigen::Vector3f v1(0, 0, 0), v2(0, 0, 0);
    float pre;

    for(int i = 0; i < 3; i++) {
        const auto& it1 = tri[i];
        const auto& it2 = tri[(i + 1) % 3];
        v1.x() = it2.x() - it1.x(), v1.y() = it2.y() - it1.y();
        v2.x() = x - it1.x(), v2.y() = y - it1.y();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for(const auto& it: v) {
        maxx = std::max(maxx, (int) floor(it.x()));
        maxy = std::max(maxy, (int) floor(it.y()));
...
void rst::rasterizer::clear(rst::Buffers buff)
{
    ...
    {
#if defined SSAA
        std::fill(depth_buf.begin(), depth_buf.end(), std::vector<float>(4, std::numeric_limits<float>::infinity()));
#else

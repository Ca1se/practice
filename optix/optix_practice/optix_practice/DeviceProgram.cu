...
{
    if (optix_launch_params.frame_id == 0 && optixGetLaunchIndex().x == 0 && optixGetLaunchIndex().y == 0) {
        // we could of course also have used optixGetLaunchDims to query
        // the launch size, but accessing the optix_launch_params here
        // makes sure they're not getting optimized away (because
        // otherwise they'd not get used)
        printf("############################################\n");
        printf("Hello world from OptiX 7 raygen program!\n(within a %ix%i-sized launch)\n",
               optix_launch_params.size.width,
               optix_launch_params.size.height);
        printf("############################################\n");
    }
    ...
    const int frame_id = optix_launch_params.frame_id;
    ...
    const uint32_t fbIndex                    = ix + iy * optix_launch_params.size.width;
    optix_launch_params.color_buffer[fbIndex] = rgba;
}
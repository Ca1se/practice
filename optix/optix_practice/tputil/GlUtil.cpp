...
initGlfw(const std::string& title, int32_t width, int32_t height)
{
    ...
    if (!glfwInit())
        throw std::runtime_error(exceptionMsg(TP_LOG_ARGS, "Failed to initialize GLFW"));
    ...
    if (window == nullptr)
        throw std::runtime_error(exceptionMsg(TP_LOG_ARGS, "Failed to create GLFW window"));
...
{
    if (!gladLoadGL())
        throw std::runtime_error(exceptionMsg(TP_LOG_ARGS, "Failed to Load GL"));
